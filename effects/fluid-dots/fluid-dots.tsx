/**
 * Fluid Dots — Interactive particle grid with water-like physics
 *
 * A GPU-accelerated dot field driven by a CPU-side fluid simulation.
 * Features dual-layer parallax, cursor-velocity wake, scroll-reactive flow,
 * proximity color shift, organic velocity sizing, and cursor-reveal masking.
 *
 * Dependencies:
 *   react, three, @react-three/fiber, @react-three/drei, next-themes
 *
 * Drop-in usage:
...
// ---------------------------------------------------------------------------
// Theme colors — adapt to your own design system
// ---------------------------------------------------------------------------

export interface FluidDotsTheme {
  dotColor: string;
  bgColor: string;
  dotOpacity: number;
}

const DEFAULT_THEMES: Record<string, FluidDotsTheme> = {
  dark: { dotColor: "#FFFFFF", bgColor: "#121212", dotOpacity: 0.025 },
  light: { dotColor: "#0F172A", bgColor: "#F7F7F8", dotOpacity: 0.35 },
};
...
function Scene({ themes }: { themes?: Record<string, FluidDotsTheme> }) {
  const size = useThree((state) => state.size);
  const viewport = useThree((state) => state.viewport);

  let activeTheme = "dark";
  try {
    // next-themes is optional — falls back to dark if unavailable
    // eslint-disable-next-line react-hooks/rules-of-hooks, @typescript-eslint/no-require-imports
    const { resolvedTheme, theme } = require("next-themes").useTheme();
    activeTheme = resolvedTheme ?? theme ?? "dark";
  } catch {
    /* next-themes not installed */
  }

  const palette = themes ?? DEFAULT_THEMES;
  const themeColors = useMemo(() => {
    return palette[activeTheme] ?? palette.dark ?? DEFAULT_THEMES.dark;
  }, [activeTheme, palette]);
...
export function FluidDotsCanvas({
  themes,
}: {
  themes?: Record<string, FluidDotsTheme>;
}) {
  return (
    <Canvas
...
export type FluidDotsBackgroundProps = {
  className?: string;
  style?: CSSProperties;
  themes?: Record<string, FluidDotsTheme>;
};

export function FluidDotsBackground({
  className,
  style,
  themes,
}: FluidDotsBackgroundProps) {
  const cls = [
