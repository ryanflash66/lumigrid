...
- **Build Tools**: Turbopack (dev) + Next.js production bundler (tree-shaking, code splitting)
- **Styling**: Tailwind CSS v4 tokens in src/app/globals.css (no PostCSS imports)
- **UI Kit**: shadcn/ui primitives, Tubelight navbar, DotShader background
- **Images**: Stored under public/assets/, ready for 
ext/image migration
- **Fonts**: 
ext/font (Geist Sans/Mono) replacing CSS @import
- **Hosting**: Vercel/Node SSR with static asset CDN caching

### File Structure
- src/app/globals.css – Tailwind base + CSS variables (optimize for minimal unused output)
- src/components/ui/*.tsx – shared primitives (ensure tree-shakeable imports)
- src/components/sections/*.tsx – high-impact sections to audit for layout shifts
- src/components/forms/contact-form.tsx – client bundle that must stay lean
- public/assets/** – hero/brand imagery; convert to WebP/AVIF as we finalize art direction
- legacy-site/ – static build for regression comparison only

### Current Performance Considerations
- Shader + Tubelight effects add client JS; ensure they suspend/defer offscreen on mobile
- Contact form validation runs client-side; keep Formspree integration async with loading states
- Unsplash placeholders inflate image weight; replace with optimized assets and 
ext/image
- Analytics/monitoring not yet installed; plan for GA4/PostHog/Sentry with script strategy
- Need real-world Lighthouse + WebPageTest baselines for the Next.js build
